<template>
  <v-app>
    ...
    <v-main class="bg-grey-lighten-3 ml-2">
      <v-row>
        <v-col v-for="(realm, i) in realms" :key="i">
          <v-card max-width="300">
            <v-card-title primary-title>
              {{ realm?.name }}
              <v-btn icon size="x-small">
                <v-icon>mdi-information-outline</v-icon>
                <v-tooltip activator="parent" :open-on-hover="false" open-on-click>
            ...
            <v-card-text>
              <v-btn rounded size="small" color="success" dark @click="showRealmCharacters(realm?.id)">角色数：{{
                realm?.numCharacters }}</v-btn>

              <v-btn rounded size="small" color="blue" class="ml-3" dark prepend-icon="mdi-currency-usd"
